...
class AssetFrame extends Dialog {
    attribute rows:Row[];
    ...
    attribute account:StockAccount on replace {
        ...
        isin=gnucashUtil.getIsinFromSymbol(symbol);
    };
    ...
    // 2 - sellMode
    // 3 - buyMode
    attribute modeSelection:Integer=0;
...
function sellAsset() {
    ...
    // take care: commodity WITH CORRECT space has to be in place before price is added
    // space could be set before without jPortfolioView
    // then we could get an error as we just use standard space
    // if (gnucashUtil.hasCommodity(GnuCashDocument.CMDTY_SPACE, symbol, isin)) {
